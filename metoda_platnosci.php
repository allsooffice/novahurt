...
$client_id = $_SESSION['id_klienta'];
	$product = $mysqli->query("SELECT * FROM sell WHERE session_id = '$client_id' AND status = 'koszyk' ORDER BY id ASC LIMIT 1");
...
if(isset($_POST['dalej']))
{
   ...
   $dodawanie = "UPDATE sell SET sposob_platnosci = '$platnosc', payment_status = '' WHERE session_id = '$client_id' AND status = 'koszyk'";
...
<body>
    ...
    <div id="tresc_informator"><a name="start"></a>
        <form method="post">
        <div class="tytuly">2/4 Wybierz metodę płatności</div>
